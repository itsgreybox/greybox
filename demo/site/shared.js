...
function generateMarkdownReport(data) {
  ...
  lines.push(`## Architecture Diagram (by folder)\n`);
  lines.push(`_A higher-level view than the file-by-file graph above: files grouped by folder, showing how those groups connect. Built from the same import data, just aggregated — not a new analysis pass._\n`);
  const archGraph = data.architecture_by_folder || {};
  if (Object.values(archGraph).some(deps => deps.length)) {
    lines.push('```mermaid');
    lines.push(buildMermaidSource(archGraph));
    lines.push('```\n');
  } else {
    lines.push(`_Everything scanned lives in one folder, or nothing links across folders — nothing to diagram here.\n`);
  }

  lines.push(`## Entry-Point Flow\n`);
  lines.push(`_Reachability from files nothing else in this scan imports (a reasonable proxy for an entry point), traced through the import graph. This is **not a traced runtime call sequence** — it can't know what actually executes first or in what order. It's an honestly-scoped signal: "if you start reading here, this is what becomes reachable," nothing more._\n`);
  const flow = data.entry_point_flow || { entry_points: [], reachable_from: {}, unreached: [] };
  flow.entry_points.forEach(entry => {
    const reached = flow.reachable_from[entry] || [];
    lines.push(reached.length
      ? `- **${entry}** reaches: ${reached.join(', ')}`
      : `- **${entry}** reaches nothing else in this scan (standalone script, or dead code worth a second look)`);
  });
  if (flow.unreached && flow.unreached.length) {
    lines.push(`\n**Not an entry point, and nothing reaches it either:** ${flow.unreached.join(', ')} — worth checking if this is actually used anywhere, or safe to remove.\n`);
  }
...
// HTML render for the entry-point flow list - matches the markdown
// export's content and honesty framing exactly, styled to match the
// rest of the report.
function renderEntryPointFlow(data) {
  const flow = data.entry_point_flow || { entry_points: [], reachable_from: {}, unreached: [] };
  const rows = flow.entry_points.map(entry => {
    const reached = flow.reachable_from[entry] || [];
    const reachedHtml = reached.length
      ? `reaches: ${reached.map(r => `<code class="inline">${escapeHtml(r)}</code>`).join(', ')}`
      : `reaches nothing else in this scan — standalone script, or dead code worth a second look`;
    return `<div class="fact-row"><b>${escapeHtml(entry)}</b> ${reachedHtml}</div>`;
  }).join('');
  const unreachedHtml = (flow.unreached && flow.unreached.length)
    ? `<div class="risk">Not an entry point, and nothing reaches it either: ${flow.unreached.map(u => `<code class="inline">${escapeHtml(u)}</code>`).join(', ')} — worth checking if this is actually used anywhere, or safe to remove.</div>`
    : '';
  return `<div class="graph-box" style="margin-top:8px;">
    <h2 style="margin-top:0;">Entry-Point Flow</h2>
    <p class="section-note">Reachability from files nothing else in this scan imports (a reasonable proxy for an entry point), traced through the import graph. This is <b>not a traced runtime call sequence</b> — it can't know what actually executes first or in what order. It's an honestly-scoped signal: "if you start reading here, this is what becomes reachable," nothing more.</p>
    ${rows || '<div class="fact-row">No clear entry point found in this sample — every file is imported by something else in the scan.</div>'}
    ${unreachedHtml}
  </div>`;
}

// Browser-side versions of the same two functions in api/_lib/analyze.js -
// live scans get these fields from the server already; this copy exists
// so sample.html's static fixture (no server round-trip) can compute
// them itself from data.modules + data.dependency_graph.
